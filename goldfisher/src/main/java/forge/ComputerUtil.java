...
public class ComputerUtil {
    ...
    public static Card getCardPreference(final Player ai, final Card activate, final String pref, final CardCollection typeList, SpellAbility sa) {
        ...
        if (pref.contains("SacCost")) {
            ...
            if (!landsInHand.isEmpty()) {
                final int numLandsInPlay = CardLists.count(ai.getCardsIn(ZoneType.Battlefield), Presets.LANDS_PRODUCING_MANA);
                final CardCollection nonLandsInHand = CardLists.getNotType(ai.getCardsIn(ZoneType.Hand), "Land");
                final int highestCMC = Math.max(6, Aggregates.max(nonLandsInHand, CardPredicates.Accessors.fnGetCmc));
    ...
    private static Card chooseCardToSacrifice(final CardCollection remaining, final Player ai, final boolean destroy) {
        ...
        if (CardLists.getNotType(remaining, "Creature").isEmpty()) {
            ...
            c = ComputerUtilCard.getWorstLand(CardLists.filter(remaining, Presets.LANDS));
        }
        else {
    ...
    public static boolean isWorseThanDraw(final Player ai, Card discard) {
        ...
        final Game game = ai.getGame();
        final CardCollection landsInPlay = CardLists.filter(ai.getCardsIn(ZoneType.Battlefield), Presets.LANDS_PRODUCING_MANA);
        final CardCollection landsInHand = CardLists.filter(ai.getCardsIn(ZoneType.Hand), Presets.LANDS);
        final CardCollection nonLandsInHand = CardLists.getNotType(ai.getCardsIn(ZoneType.Hand), "Land");
        final int highestCMC = Math.max(6, Aggregates.max(nonLandsInHand, CardPredicates.Accessors.fnGetCmc));
    ...
    public static boolean scryWillMoveCardToBottomOfLibrary(Player player, Card c) {
        ...
        CardCollection landsOTB = CardLists.filter(cardsOTB, Presets.LANDS_PRODUCING_MANA);
        CardCollection thisLandOTB = CardLists.filter(cardsOTB, CardPredicates.nameEquals(c.getName()));
        CardCollection landsInHand = CardLists.filter(cardsInHand, Presets.LANDS_PRODUCING_MANA);
        ...
        CardCollectionView allCreatures = CardLists.filter(allCards, Presets.CREATURES, CardPredicates.isOwner(player));
        ...
        if (c.isLand()) {
            ...
            CardCollection creaturesOTB = CardLists.filter(cardsOTB, Presets.CREATURES);
        ...
        case "DeathOrTaxes":
            ...
            if (opponent) {
                ...
                if (CardLists.count(aiCreatures,
                        Predicates.or(CardPredicates.hasSVar("SacMe"), Presets.TOKEN)) >= numDeath) {
                    return "Death";
                }
